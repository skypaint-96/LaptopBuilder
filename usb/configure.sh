...
allowed_key() {
  case "$1" in
    DISK|ESP_SIZE_MIB|HOSTNAME|USERNAME|TIMEZONE|LOCALE|KEYMAP|CPU_VENDOR|GPU_VENDOR|KERNELS|FILESYSTEM|DESKTOP|LUKS_PASSPHRASE_FILE|USER_PASSWORD_FILE|TPM_PIN_FILE|INITIAL_INSTALL_SOURCE_MODE|ENABLE_MULTILIB|ENABLE_SSD_TRIM|ENABLE_AUR|AUR_HELPER|AUR_HELPER_PACKAGE|AUR_PACKAGES|AUR_NONINTERACTIVE|PROVISION_NONINTERACTIVE|ENABLE_DOCKER|DOCKER_ADD_USER_TO_GROUP|ENABLE_GAMING|ENABLE_SNAPSHOTS|ENABLE_T480|ENABLE_POWERSHELL_PROFILE|INSTALL_POWERSHELL_MODULES|INSTALL_VSCODE_EXTENSIONS|ENABLE_BLUETOOTH|ENABLE_SSH|ENABLE_SECURE_BOOT|REQUIRE_SETUP_MODE_AT_INSTALL|AUTO_PREPARE_SECURE_BOOT|SBCTL_ENROLL_MICROSOFT|ENABLE_TPM|REQUIRE_TPM|TPM_PCRS|TPM_WITH_PIN|TPM_PIN_MIN_LENGTH|TPM_PIN_NUMERIC_ONLY|STAGE_TPM_CREDENTIALS|T480_BATTERY_THRESHOLDS|T480_START_CHARGE|T480_STOP_CHARGE) return 0 ;;
    *) return 1 ;;
  esac
}
